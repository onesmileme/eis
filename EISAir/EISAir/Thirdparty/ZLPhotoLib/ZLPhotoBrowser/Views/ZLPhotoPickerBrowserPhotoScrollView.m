...
@property (nonatomic , strong) ZLPhotoPickerBrowserPhotoImageView *photoImageView;
@property (nonatomic , strong) ZLPhotoPickerBrowserPhotoView *tapView; // for background taps
@property (nonatomic , assign) BOOL loadImageSuccess;//加载图片是否成功
...
- (void)setPhoto:(ZLPhotoPickerBrowserPhoto *)photo{
    ...
    _loadImageSuccess = false;
    
    __weak typeof(self) weakSelf = self;
    
    if (photo.asset.asset && photo.photoURL.absoluteString.length == 0) {
        photo.photoURL = [photo.asset.asset valueForProperty:ALAssetPropertyAssetURL];
    }
    ...
    if (photo.photoURL) {
        ...
        if (photoRange.location != NSNotFound){
            [[ZLPhotoPickerDatas defaultPicker] getAssetsPhotoWithURLs:photo.photoURL callBack:^(UIImage *obj) {
                ...
                weakSelf.loadImageSuccess = true;
            }];
        }else{
            ...
            [_photoImageView sd_setImageWithURL:photo.photoURL placeholderImage:thumbImage options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                ...
                if (image) {
                    ...
                    if (!image) {
                        image = [UIImage imageNamed:@"image_load_failed"];
                        weakSelf.loadImageSuccess = false;
                    }
                    weakSelf.photoImageView.image = image;
                    [weakSelf displayImage];
                    ...
                    [weakSelf.photoImageView removeScaleBigTap];
...
- (CGRect )setMaxMinZoomScalesForCurrentBounds:(UIImageView *)imageView {
    ...
    if (xScale < 1 && yScale < 1 && yScale <= xScale + 0.01) {
        //都放大但 是超长图
